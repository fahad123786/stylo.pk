...
const Summer24 = () => {
  ...
  // Fetch products for Summer24 collection
  const fetchData = async () => {
    setLoader(true);
    try {
      const response = await axios.get("http://localhost:8082/api/admin/product?category=summer24");
      setProducts(response.data.products);
      console.log(response);
    } catch (error) {
      console.error('Error fetching Summer24 products', error);
    }
    setLoader(false);
  };
